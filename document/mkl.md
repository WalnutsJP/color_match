# Monge-Kantorovich Linearization (MKL)
...
3. 平均・共分散行列を利用して以下の計算で色変換  
  変換行列 $A$ は以下のように定義される：
  $$
  A = \Sigma_{\text{src}}^{-1/2} \sqrt{\Sigma_{\text{src}}^{1/2} \Sigma_{\text{ref}} \Sigma_{\text{src}}^{1/2}} \Sigma_{\text{src}}^{-1/2}
  $$  
  色変換は以下の式で行われる：
  $$
...
- $\boldsymbol{\Sigma}_{\text{src}}^{1/2}$ : 入力画像の共分散行列の平方根
- $\boldsymbol{\Sigma}_{\text{ref}}^{-1/2}$ : 参照画像の共分散行列の逆平方根
...
- MVGDとは変換行列A部分の計算のみ異なり、MVGDは色相の回転が生じる可能性があるのに対し、MKLは回転成分が最小化される為、色相を変えずに明度・彩度のみ調整することが出来る
